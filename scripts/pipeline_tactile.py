...
def main(args):
    ...
    test_dir = os.path.join(
        ...
        datetime.now().strftime("%d_%m_%H%M%S_") + str(random.randint(0, 10000)),
    )
    ...
    weights_path = os.path.join(
        os.path.dirname(runs_touch.__file__), args["folder_touch"], "weights.pt"
    )
    touch_model.load_state_dict(
    ...
    vertices_wrld = (
        utils_mesh.rotate_pointcloud(mesh_original.vertices, initial_obj_rpy)
        * args["scale"]
        + initial_obj_pos
    )
    ...
    verts_deepsdf = utils_mesh.rotate_pointcloud(
        mesh_original.vertices, initial_obj_rpy
    )
    trimesh.Trimesh(vertices=verts_deepsdf, faces=mesh_original.faces).export(
        os.path.join(test_dir, "mesh_deepsdf.obj")
    )
    ...
    while True:
        ...
        robot = utils_sample.robot_touch_spherical(
            robot, robot_sphere_wrld, initial_obj_pos, angles
        )
        ...
        contact_pointcloud = utils_raycasting.filter_point_cloud(
            robot.results_at_touch_wrld, obj_id
        )
        check_on_contact_pointcloud = utils_sample.check_on_contact_pointcloud(
            contact_pointcloud, 1
        )
        if not check_on_contact_pointcloud:
            print(
                f"Point cloud shape is too small: {contact_pointcloud.shape[0]} points"
            )
            # pb.removeBody(robot.robot_id)
            continue
        ...
        predicted_mesh = trimesh.Trimesh(
            predicted_verts.detach().cpu(), initial_faces.cpu()
        )
        ...
        predicted_pointcloud_wrld = utils_mesh.translate_rotate_mesh(
            pos_wrld, rot_M_wrld, predicted_pointcloud[None, :, :], initial_obj_pos
        )
        ...
        pointcloud_deepsdf = (
            torch.from_numpy(pointcloud_deepsdf_np).float().to(device)
        )  # shape (n, 3)
        pointclouds_deepsdf = torch.vstack((pointclouds_deepsdf, pointcloud_deepsdf))

        # The sdf of points on the object surface is 0.
        sdf_gt = torch.vstack(
            (sdf_gt, torch.zeros(size=(pointcloud_deepsdf.shape[0], 1)).to(device))
        )
        ...
        if args["augment_points_num"] > 0:
            ...
            rpy_wrld = np.array(
                robot.arm.get_current_TCP_pos_vel_worldframe()[1]
            )  # TCP orientation
            normal_wrk = np.array([[0, 0, 1]])
            normal_wrld = utils_mesh.rotate_pointcloud(normal_wrk, rpy_wrld)[0]
            TCP_pos_deepsdf = (
                robot.arm.get_current_TCP_pos_vel_worldframe()[0] - 0.01 * normal_wrld
            )  # move it slightly away from the object
            TCP_pos_deepsdf = (TCP_pos_deepsdf - initial_obj_pos) / args[
                "scale"
            ]  # convert to DeepSDF scale
            sensor_dirs = TCP_pos_deepsdf - pointcloud_deepsdf_np

            # Estimate point cloud normals
            _, n = pcu.estimate_point_cloud_normals_knn(
                pointcloud_deepsdf_np, 32, view_directions=sensor_dirs
            )

            # Sample along normals and return points and distances
            (
                pointcloud_along_norm_np,
                signed_distance_np,
            ) = utils_sample.sample_along_normals(
                std_dev=args["augment_points_std"],
                pointcloud=pointcloud_deepsdf_np,
                normals=n,
                N=args["augment_points_num"],
                augment_multiplier_out=args["augment_multiplier_out"],
            )

            pointcloud_along_norm = (
                torch.from_numpy(pointcloud_along_norm_np).float().to(device)
            )
            sdf_normal_gt = torch.from_numpy(signed_distance_np).float().to(device)

            pointclouds_deepsdf = torch.vstack(
                (pointclouds_deepsdf, pointcloud_along_norm)
            )
        ...
        if args["render_scene"]:
            ...
            for idx_camera, cameraEyePosition in enumerate(cameraEyePositions):
                ...
                rgb_image = utils_sample.render_scene(
                    view_matrix, projection_matrix, width, height
                )
                # Save image
                plt.imsave(
                    os.path.join(image_dir, f"camera_{idx_camera}.png"), rgb_image
                )
